...
plotIncubationCDF <- function(distro,infPer,fillCol="lightsteelblue1",infCol="orangered2") with(distro, {
	browser()
  par(bty="L",lwd=3)
	par(mar=c(5,7,1,1)+1)
	...
	abline(v = infPer, col=infCol,lwd=3)
	legend("right",legend = c("Cumulative probability","Normalized daily probability","Infectious period"),bty="n",cex=1.3,
				 lwd=c(6,NA,3),lty=c(1,NA,1),pch=c(NA,22,NA),col=c("black",fillCol,infCol),pt.bg=c(NA,fillCol,NA),pt.cex=1.5)
})
