...
tabsetPanel(
  tabPanel("Cumulative Incidence", renderPlot({ plotCumInc(res(), start=input$start_date, reporting_rate=input$reporting_rate) })),
  tabPanel("Bed Occupancy", renderPlot({ plotBedOcc(res(), start=input$start_date, reporting_rate=input$reporting_rate) }))
)
```
...
buildPDFPanel <- function(prefix, full,
  ...
  column(4,
    ...
    conditionalPanel("input.pdf_type == 'manual'", div(class = "pdf",
      div(paste0("Total ", full," "), em("p: "), textOutput(sum_check_id, span)),
      div(paste0("day - ", full," p")),
      manualPDF(prefix)
    )),
...
with(defaults, {
  tabsetPanel(
    ...
    tabPanel("Scenario Parameters", inputPanel(
      fixedRow(
        buildInterventionCol(),
        buildIntroductionCol()
      )
    )),
    tabPanel("Notes", renderText(
...
tabsetPanel(
  ...
  tabPanel("Introductions",
    renderTable(introduction_scenario(), inline=T),
    renderText("TODO plot")
  ),
  tabPanel("Intervention Scenario",
    renderTable(intervention_scenario(), inline=T),
    renderPlot({
