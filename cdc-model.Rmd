...
buildPDFPanel <- function(prefix, full,
  ...
  column(3,
    numericInput(paste0(prefix,"_period_max"), paste0(full," Period Max (days)?"), min=1, max=absolute_pdf_day_max, value=def_period_max, step=1),
    conditionalPanel("input.pdf_type == 'manual'",
      renderText(paste0(full," Day")),
      manualPDF(prefix)
    ),
    conditionalPanel("input.pdf_type == 'functional'",
      selectInput(paste0(prefix,"_model"),paste0(full," Distribution"), choices=choices, selected=def_model),
      numericInput(paste0(prefix,"_period_mean"), paste0(full, " Period Mean (days)?"), min=0.01, max=25, value=def_period_mean, step=0.01),
      numericInput(paste0(prefix,"_period_sd"), paste0(full, " Period St.Dev (days)?"), min=0.01, max=25, value=def_period_sd, step=0.01)
    )
  )
}
with(defaults, {
  inputPanel(
    ...
    fixedRow(
      ...
      column(3,
        renderText("Verifications: "),
        renderText(paste0("Total Inc. Day P: ", sum(distros$inc()$pdf))),
        renderText(paste0("Total Inf. Day P: ",sum(distros$inf()$pdf))),
        renderText("Hospital Resource Parameters")
      )
    ),
    fixedRow(
...
treatment_durations <- reactive({
  ref <- paste0("tr_dur_", names(defaults$infective_treatments))
  ...
  validate(
    need(all(res == floor(res)), "treatment durations not integers.")
  )
  return(res)
})
