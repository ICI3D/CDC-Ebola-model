...
author: "CAB Pearson (model), JRC Pulliam (plots)"
date: "2014 NOV 06"
output:
  html_document:
    self_contained: no
...
with(defaults, {
  tabsetPanel(
    ...
    tabPanel("Scenario Parameters",
      renderText("Input an introduction scenario"),
      fileInput('introduction_file', 'Choose file to upload',
        accept = accepted_file_types
      ),
      renderText("Input an intervention scenario"),
      fileInput('intervention_file', 'Choose file to upload',
        accept = c(
          'text/csv',
...
buildFileReactive <- function(name, defname) {
  return(reactive({
    src <- if (is.null(input[[name]])) {
      defname
    } else {
      input[[name]]$datapath
    }
    return(read.table(src, header=T))
  }))
}
intervention_scenario <- buildFileReactive('intervention_file', "./cdc_scenario1_distros.csv")
introduction_scenario <- buildFileReactive('introduction_file', "./cdc_scenario1_intros.csv")
```
...
tabsetPanel(
  ...
  tabPanel("Intervention Scenario",
    ...
    renderPlot({
      p <- plotInterventionScenario(intervention_scenario(), defaults$simulation_duration)
      p
    })
  )
)
