...
incubation_entries <- manualPDF("inc")
infectious_entries <- manualPDF("inf")
with(defaults, {
  inputPanel(
    ...
    radioButtons("pdf_type","Distribution Specification: ", choices=c("manual","functional"), selected="manual", inline=T),
    fixedRow(
      ...
      column(3, numericInput("inf_period_max", "Infectious Period Max (days)?", min=1, max=absolute_pdf_day_max, value=inf_period_max, step=1),
        conditionalPanel("input.pdf_type == 'manual'",
      ...
      column(3, renderText("Verifications: "), renderText(paste0("Total Inc. Day P: ", sum(inc_distro()$pdf))), renderText(paste0("Total Inf. Day P: ",sum(inf_distro()$pdf))))
    ),
    fixedRow(
